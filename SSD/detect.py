...
def detect(original_image, min_score, max_overlap, top_k, suppress=None):
    ...
    for i in range(det_boxes.size(0)):
        ...
        # Text
        text_size = font.getsize(det_labels[i])
        text_location = [box_location[0] + 2., box_location[1] - text_size[1]]
        textbox_location = [box_location[0], box_location[1] - text_size[1], box_location[1] + text_size[0] + 4.,
                            box_location[1]]
        #draw.rectangle(xy=textbox_location, fill=label_color_map[det_labels[i]])
        draw.text(xy=text_location, text=det_labels[i], fill=label_color_map[det_labels[i]],
                  font=font)
    del draw
...
if __name__ == '__main__':
    img_path = 'imgpruebas\E4iK3FzWEAE2qrs.jpg'
